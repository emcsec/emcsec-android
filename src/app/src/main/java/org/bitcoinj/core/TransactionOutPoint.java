...
public class TransactionOutPoint extends ChildMessage {
     ...
     */
    @Nullable
    public ECKey getConnectedKey(KeyBag keyBag) throws org.bitcoinj.script.ScriptException {
        ...
        if (ScriptPattern.isPayToPubKeyHash(connectedScript)) {
            ...
            throw new org.bitcoinj.script.ScriptException(ScriptError.SCRIPT_ERR_UNKNOWN_ERROR, "Could not understand form of connected output script: " + connectedScript);
        }
    }
     ...
     */
    @Nullable
    public RedeemData getConnectedRedeemData(KeyBag keyBag) throws org.bitcoinj.script.ScriptException {
        TransactionOutput connectedOutput = getConnectedOutput();
        checkNotNull(connectedOutput, "Input is not connected so cannot retrieve key");
        Script connectedScript = connectedOutput.getScriptPubKey();
        if (ScriptPattern.isPayToPubKeyHash(connectedScript)) {
