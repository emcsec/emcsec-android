...
public class ScriptPattern {
    ...
    public static boolean isPayToWitnessHash(Script script) {
        ...
        Log.d("isPayToWitnessHash", "isPayToWitnessHash: ");
        List<ScriptChunk> chunks = script.chunks;
        if (chunks.size() != 2) {
            Log.d("isPayToWitnessHash", "chunks.size() != 2");
            return false;
        }
        if (!chunks.get(0).equalsOpCode(OP_0)) {
            Log.d("isPayToWitnessHash", "!chunks.get(0).equalsOpCode(OP_0)");
            return false;
        }
        byte[] chunk1data = chunks.get(1).data;
        if (chunk1data == null){
            Log.d("isPayToWitnessHash", "chunk1data == null");
            return false;
        }
        if (chunk1data.length != SegwitAddress.WITNESS_PROGRAM_LENGTH_PKH
                && chunk1data.length != SegwitAddress.WITNESS_PROGRAM_LENGTH_SH){
            Log.d("isPayToWitnessHash", "chunk1data.length != SegwitAddress.WITNESS_PROGRAM_LENGTH_PKH\n" +
                    "                && chunk1data.length != SegwitAddress.WITNESS_PROGRAM_LENGTH_SH");
            return false;
        }
        return true;
